...
import { isNotNil } from '@extension/core';
...
type Props = {
  ...
  Pick<React.HTMLProps<HTMLDivElement>, 'id' | 'onClick' | 'onMouseEnter' | 'onMouseLeave' | 'style'>;

const Flex = styled($Flex)<Props>`
  margin: ${({ theme, margin, marginHorizontal, marginVertical, marginTop, marginBottom, marginLeft, marginRight }) => {
    switch (true) {
      ...
      case isNotNil(marginTop) || isNotNil(marginBottom) || isNotNil(marginLeft) || isNotNil(marginRight):
        return `${marginTop ? theme.spacing[marginTop] : 0} ${
          marginRight ? theme.spacing[marginRight] : 0
        } ${marginBottom ? theme.spacing[marginBottom] : 0} ${marginLeft ? theme.spacing[marginLeft] : 0}`;
      default:
        return '0';
    }
  }};

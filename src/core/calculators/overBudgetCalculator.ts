    ...
    const catagories = data.budgetData.monthlyAmountsByCategory
      ...
      .map((x) => { 
        return {
          ...
          plannedCashFlowAmount: round ? Math.round(x.monthlyAmounts[0].plannedCashFlowAmount) : x.monthlyAmounts[0].plannedCashFlowAmount, 
          actualAmount: round ? Math.round(x.monthlyAmounts[0].actualAmount) : x.monthlyAmounts[0].actualAmount, 
          remainingAmount: round ? Math.round(x.monthlyAmounts[0].remainingAmount) : x.monthlyAmounts[0].remainingAmount}
      })
